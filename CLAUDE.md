...
- `./Scripts/package_app.sh` — Release build → sign → `dist/Burnbar-vX.Y.Z.zip` (prints sha256) (1.6.1). Ad-hoc by default (no hardened runtime; right-click → Open on first launch); with `DEVELOPER_ID_APPLICATION` set it does Developer ID + hardened-runtime signing, and notarizes+staples when notary creds are present (#167).
...
- `./Scripts/make_dmg.sh` — wrap the signed `Burnbar.app` (reused from a prior Release build, else built) into a drag-to-Applications `dist/Burnbar-vX.Y.Z.dmg` (prints sha256) (1.6.2). The DMG contains `Burnbar.app` + an `/Applications` symlink (drag-to-install). Ad-hoc by default; with `DEVELOPER_ID_APPLICATION` set it Developer-ID-signs the DMG container and notarizes+staples it when notary creds are present (#167). Prefers `create-dmg` (`brew install create-dmg`) for a styled window; falls back to plain `hdiutil` if create-dmg is absent or its Finder styling step fails (e.g. headless/CI). Drag-install works either way.
- **Release pipeline** — pushing a `v*.*.*` tag triggers `.github/workflows/release.yml` (on `macos-14`): it runs `package_app.sh` then `make_dmg.sh` (the DMG reuses the zip's signed build) and creates a GitHub Release with **both** the `.zip` and `.dmg` attached. Signing is automatic by environment (#167): when the `MACOS_CERTIFICATE` + notary secrets are configured it does Developer ID + hardened-runtime signing + notarization + stapling; otherwise it falls back to ad-hoc (right-click → Open). Operator setup is in `docs/M3-OPERATOR-SETUP.md` §6.
- `./Scripts/sign_and_notarize.sh <sign-app|notarize|staple-app|can-sign|can-notarize> [path]` — Developer ID + hardened-runtime signing and notarization/stapling, gated on `DEVELOPER_ID_APPLICATION` + notary creds (`APPLE_ID`/`APPLE_TEAM_ID`/`APPLE_APP_SPECIFIC_PASSWORD` or `NOTARY_KEYCHAIN_PROFILE`). Called by `package_app.sh`/`make_dmg.sh`; no-op fallback to ad-hoc when the env vars are absent. Entitlements: `Scripts/Burnbar.entitlements` (#167).
