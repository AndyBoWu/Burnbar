...
- `./Scripts/make_dmg.sh` — wrap the ad-hoc-signed `Burnbar.app` (reused from a prior Release build, else built) into a drag-to-Applications `dist/Burnbar-vX.Y.Z.dmg` (prints sha256) (1.6.2). The DMG contains `Burnbar.app` + an `/Applications` symlink (drag-to-install). Prefers `create-dmg` (`brew install create-dmg`) for a styled window; falls back to plain `hdiutil` if create-dmg is absent or its Finder styling step fails (e.g. headless/CI). Drag-install works either way.
- **Release pipeline** — pushing a `v*.*.*` tag triggers `.github/workflows/release.yml` (on `macos-14`): it runs `package_app.sh` then `make_dmg.sh` (the DMG reuses the zip's signed build) and creates a GitHub Release with **both** the `.zip` and `.dmg` attached. Ad-hoc signing only (no notarization yet); the Developer ID / notarization step (#167) slots in at the marked insertion point in that file.
